...
public class Contrato extends javax.swing.JFrame {
    ...
    private void btnFillActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFillActionPerformed
            ...
            Connection conn= DriverManager.getConnection("jdbc:oracle:thin:@165.227.213.232:1521:dbportafolio","C##_equipo2","portafolio02");
            Statement sent = conn.createStatement();
            ResultSet rs7 = sent.executeQuery("select monto_meta from curso where curso_id ='"+curso_id+"'");
            while(rs7.next()){this.txtMont.setText(rs7.getString("monto_meta"));}
            conn.close();
            }catch(SQLException e)
            {
                e.printStackTrace();
            ...
            Connection conn= DriverManager.getConnection("jdbc:oracle:thin:@165.227.213.232:1521:dbportafolio","C##_equipo2","portafolio02");
            Statement sent = conn.createStatement();
            ResultSet rs7 = sent.executeQuery("Select cue.apellido_p from cuenta cue join curso cu on cue.cuenta_id= cu.cuenta_id  where cu.curso_id='"+curso_id+"'");
            while(rs7.next()){this.txtRepCursoAP.setText(rs7.getString("apellido_p"));}
            conn.close();
            }catch(SQLException e)
            {
                e.printStackTrace();
            ...
            Connection conn= DriverManager.getConnection("jdbc:oracle:thin:@165.227.213.232:1521:dbportafolio","C##_equipo2","portafolio02");
            Statement sent = conn.createStatement();
            ResultSet rs7 = sent.executeQuery("Select cue.apellido_m from cuenta cue join curso cu on cue.cuenta_id= cu.cuenta_id  where cu.curso_id='"+curso_id+"'");
            while(rs7.next()){this.txtRepCursoAM.setText(rs7.getString("apellido_m"));}
            conn.close();
            }catch(SQLException e)
            {
                e.printStackTrace();
